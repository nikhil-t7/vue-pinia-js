...
import { NewUser, User ,SignInUser,userSingIn} from "../users";
...
const allUsers: User[] = [];
const allSignInUser: userSingIn[] = []
...
app.post<{}, {}, SignInUser>("/login", (req, res) => {
  const targetUser = allSignInUser.find((x) => x.username === req.body.username);
...
app.post<{}, {}, NewUser>("/login", (req, res) => {
  const targetUser = allUsers.find((x) => x.firstName === req.body.firstName);
